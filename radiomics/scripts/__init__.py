...
class MyPyRadiomicsCommandLine:
    ...
    def getParser(cls):
        ...
        inputGroup.add_argument('--setting', '-s', metavar='"SETTING_NAME:VALUE"', action='append', default=[],
                                type=str,
        ...
        outputGroup = parser.add_argument_group(title='Output',
        ...
        for case_idx, case in case_generator:
            if case_idx == 1 and self.args.param is not None:
                if not os.path.isfile(self.args.param):
                    ...
                    c = pykwalify.core.Core(source_file=self.args.param, schema_files=[schemaFile],
        ...
        for setting in self.args.setting:  # setting = "setting_key:setting_value"
            ...
            try:
                ...
                if 'seq' in setting_def:
                    ...
                    setting_overrides[setting_key] = [parse_value(val, setting_type) for val in
                                                      setting_value.split(',')]
                ...
                self.logger.warning('Could not parse value "%s" for setting "%s", skipping...', setting_value,
                                    setting_key)
        ...
        if self.args.log_file is not None:
            ...
            if self.args.jobs > 1 and py_version >= (3, 2):
                ...
                file_handler.setFormatter(logging.Formatter(fmt=logging_config['formatters']['default'].get('format'),
                                                            datefmt=logging_config['formatters']['default'].get(
                                                                'datefmt')))
